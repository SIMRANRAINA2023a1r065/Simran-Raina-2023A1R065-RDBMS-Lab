...
desc Instructors;
/*column name = expression name automatically*/
select name,salary*12 from Instructors;
desc Instructors;
/*column aliasing*/
select name,salary*12 asal from instructors;
desc instructors;
/* If in space put in double quotes*/
select name,salary*12 "Annual Salary" from Instructors;
desc instructors;
/*change built in column names but table main no change*/
select name "Instructor Name", salary*12 "Annual Salary" from Instructors;
desc Instructors;
select* from Instructors;
/*How to supress duplicascy*/
select distinct deptid, name from Instructors;
/*Combination unique 2 bar alice nhi chalega*/
select* /*all clauses in different line can be written also*/
from students
where deptid=5;
select* /*all clauses in different line can be written also*/
from students
...
select*
from students
where deptid=5 and gender ='female'; /* both the conditions needs to be true*/
select* /*all clauses in different line can also be written*/
from students
where deptid=5 or gender ='female'; /* atleast 1 condition needs to be true from 2 conditions*/
use university;
select*
from students
...
select name,salary from instructors
 ...
 /*Range main 10 bhi ayega , yaha in ayega*/
 select*
 from instructors
 where deptid in(1,2,3,4,5) and(salary between 80000 and 85000);/*in is a keyword for required data to shown*/
 select*
 from instructors
 where deptid not in(1,2,3,4,5) and (salary between 80000 and 85000);/* "not in " is a keyword for not showing that particular data*/
 select* from students;
 select* from students
 where name='oliver Miller';/* if you wnat to get the record of the particular person or of any data
 /*underscore can hide one character but % can hide more than one char which is: 'like keyword'*/
 select name from students
 where name like '0%';    /*name=output*/
 select name from students
 where name like '%n';   /*name=12 names it shows*/
 select name from students
 where name like '0%n';   /*output=name*/
 /*2nd last character =a*/
 select name from students
 where name like '%a_'; /* Output=Benjamin Thomas
                                  Aurora Morgan*/
/*3rd last character=a*/
select* from students
where name like '%a__';
/* If in between there is underscore*/
select name from students
where name like '%_%';  /*Here underscore meaning is space*/
/*But if you wnat to see underscore only*/
select* from students
where name like'%\_%'; /*NULL-Nothing showing in Output*/
select* from students
where name like '%\%%'; /* If you want to see % fully */
