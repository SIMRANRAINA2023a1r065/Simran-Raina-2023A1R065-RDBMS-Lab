...
desc emplog;
-- primary key is a build in triggers , all the constraints are build - in Triggers
-- creating Trigger
delimiter //
drop trigger empbackup//
create trigger empbackup before 
delete on employees for each row
begin
       declare un varchar(20);
       set un=user();
       insert into emplog values(old.eid,old.name,old.salary,old.email,old.gender,un,now()); 
       
end //

-- old is qualifier here
       -- jb record insert krte hai toh hmme new values milti hai, aur delete k case m hmme old values milti hai and update m hmme dono values milti hai new bhi aur old bhi
delimiter ;
select* from emplog;
rollback;
select* from employees;
